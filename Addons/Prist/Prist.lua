...
local CD = BFCooldown
CD.style = {"Interface\\Cooldown\\star4", "Interface\\Cooldown\\ping4", "Interface\\Cooldown\\starburst"}
CD.Util = {}
local isload
print(_G.SpellTimerFrameBFCD)
function CD.Util:GetSpellID(spell)
    if not spell then
        return
    end
    if not BFCooldownDB.spellCache or not isload then
        BFCooldownDB.spellCache = {}
        isload = true
    end
    if BFCooldownDB.spellCache[spell] then
        if BFCooldownDB.spellCache[spell][2] == "spell" then
            if
                type(BFCooldownDB.spellCache[spell][1]) == "number" and
                    (GetSpellInfo(BFCooldownDB.spellCache[spell][1], "spell")) == spell
             then
                return unpack(BFCooldownDB.spellCache[spell])
            end
        elseif BFCooldownDB.spellCache[spell][2] == "pet" then
            if
                type(BFCooldownDB.spellCache[spell][1]) == "number" and
                    (GetSpellInfo(BFCooldownDB.spellCache[spell][1], "pet")) == spell
             then
                return unpack(BFCooldownDB.spellCache[spell])
            end
        else
    ...
    for i = 1, offset + numSpells do
        ...
        if s == spell then
            BFCooldownDB.spellCache[spell] = {i, "spell"}
            return i, "spell"
        end
    end
    local slots, id
    for i = 1, 19 do
        ...
        if id then
            ...
            if s and s == spell then
                BFCooldownDB.spellCache[spell] = {id, "item"}
                return id, "item"
            end
        end
    end
...
function CD.Util:GetCD(id, type)
    ...
    if type == "spell" or type == "pet" then
        start, duration, enable, modRate = GetSpellCooldown(id, type)
    elseif type == "item" then
...
function CD.Util:GetFormattedTime(t, long)
    ...
    if t < 9 then
        style = BFCooldownDB.short
        str = ceil(t)
        nextUpdate = t - floor(t)
    elseif t < 60 then
        style = BFCooldownDB.secs
        str = ceil(t)
        nextUpdate = t - floor(t)
    elseif t < 3600 then
        style = BFCooldownDB.mins
    ...
    elseif (t < 86400) then
        style = BFCooldownDB.hrs
        str = format("%dh", ceil(t / 3600))
        nextUpdate = t % 3600
    else
        style = BFCooldownDB.days
...
function CD.Util:FormatCenterText(text)
    ...
    return format(
        ...
        format(
            "|cff%2x%2x%2x%s|r",
            BFCooldownDB.center.r * 255,
            BFCooldownDB.center.g * 255,
            BFCooldownDB.center.b * 255,
            text
        )
    )
end
...
function CD.Util:GetTimeString(BFCooldown_ac970a92b1bae84f591d2098084a233d)
    BFCooldown_ac970a92b1bae84f591d2098084a233d = math.floor(BFCooldown_ac970a92b1bae84f591d2098084a233d)
    local BFCooldown_377682ab4d11f1b730b680e0c65e174c = math.floor(BFCooldown_ac970a92b1bae84f591d2098084a233d / 60)
    local BFCooldown_c7b591b2c752c8cb9c847770ee2d8f9d =
        BFCooldown_ac970a92b1bae84f591d2098084a233d - BFCooldown_377682ab4d11f1b730b680e0c65e174c * 60
    return string.format(
        "%02d:%02d",
        BFCooldown_377682ab4d11f1b730b680e0c65e174c,
        BFCooldown_c7b591b2c752c8cb9c847770ee2d8f9d
    )
end
local OO = {}
local function spellEqual(s1, s2)
    if (s1.name and s2.name and s1.name == s2.name) or (s1.spell and s2.spell and s1.spell == s2.spell) then
        return true
    end
    return false
end
OO.Spell = {}
function OO.Spell:New(spellID)
    local t = {}
    setmetatable(t, {__index = OO.Spell, __eq = spellEqual})
    t.spell = GetSpellInfo(spellID)
    return t
end
...
function OO.Spell:IsValid()
    if self.id and self.duration and self.duration > 2 then
        return true
    end
    return false
end
function OO.Spell:IsExpired()
    if not self.left or self.left <= 0 then
        return true
    end
end
...
local function Center_Update(self, elapsed)
    ...
    local alpha = BFCooldownDB.center.alpha
    if self.finish > BFCooldownDB.center.time / 2 then
        alpha = (1 - self.finish / BFCooldownDB.center.time) * 2 * alpha
    end
    self:SetAlpha(alpha)
    if self.finish >= BFCooldownDB.center.time then
...
function CD:UpdateCenterStyle(frame)
    if frame then
        frame:SetWidth(BFCooldownDB.center.width)
        frame:SetHeight(BFCooldownDB.center.width)
        frame:SetAlpha(BFCooldownDB.center.alpha)
        frame:ClearAllPoints()
        frame:SetPoint(
            BFCooldownDB.center.position.p,
            UIParent,
            BFCooldownDB.center.position.r,
            BFCooldownDB.center.position.x,
            BFCooldownDB.center.position.y
        )
        frame:SetBackdrop {
        ...
        frame.text:SetFont(BFCooldownDB.center.font, BFCooldownDB.center.size, "OUTLINE")
        if BFCooldownDB.center.text then
            frame.text:Show()
        else
            frame.text:Hide()
        end
        frame.icon:SetBlendMode(BFCooldownDB.center.mode and "ADD" or "BLEND")
    end
end
function CD:UpdateCenter()
    if BFCooldownDB.center.config and centerList:Count() > 0 and (not self.center:IsVisible()) then
...
function CD:UpdateShine(shine, elapsed)
    ...
    local scale = (BFCooldownDB[shine.type].scale - 1) * (1 - shine.completed) + 1
    if scale <= 1 then
        actives[shine] = nil
        shine:Hide()
    else
...
local function IsSpecialButton(button, type)
    if type == "ACTION" then
    ...
    if BFCooldownDB.ACTION.special then
        return false
    end
...
function CD:SetCooldown(cooldown, start, duration)
    ...
    if cooldown.button then
        ...
        if not IsSpecialButton(cooldown.button, cooldown.type) then
            if cooldown.type then
                if start > 0 and duration > 2 and BFCooldownDB[cooldown.type].config then
...
function CD:UpdateTimer(timer)
    timer.cooldown:SetAlpha(BFCooldownDB[timer.cooldown.type].hidecooldown and 0 or 1)
    local _time = timer.duration - (GetTime() - timer.start)
    local max = BFCooldownDB[timer.cooldown.type].max
    if max and max > 0 and _time > max then
        timer.text:Hide()
    elseif _time > 0.4 then
        local str, scale, r, g, b, nextUpdate =
            self.Util:GetFormattedTime(_time, BFCooldownDB[timer.cooldown.type].long)
        local size = timer:GetWidth() or timer.cooldown.button:GetWidth()
        size = floor(size / 36 * BFCooldownDB[timer.cooldown.type].size * scale)
        if size <= 0 then
            timer.nextUpdate = 0.2
        else
            timer.text:SetFont(BFCooldownDB[timer.cooldown.type].font, size, "OUTLINE")
            timer.text:SetText(str)
            timer.text:SetTextColor(r, g, b)
            timer.text:SetAlpha(BFCooldownDB[timer.cooldown.type].alpha)
            timer.text:SetPoint("CENTER", timer, BFCooldownDB[timer.cooldown.type].point or "CENTER", 0, 0)
            timer.text:Show()
            timer.nextUpdate = nextUpdate
        end
    else
        timer:Hide()
        if _time > -1 and BFCooldownDB[timer.cooldown.type].shine then
...
function CD:CreateTimer(cooldown)
    ...
    text:SetPoint("CENTER", timer, BFCooldownDB[cooldown.type].point or "CENTER", 0, 0)
    timer.text = text
...
function CD.Timer.TF:Update()
    if BFCooldownDB.bar and #(self.timers) > 0 then
        self.ui:Show()
        for _i, _timer in pairs(self.timers) do
...
function CD.Timer.ST:Create(spell)
    ...
    local ui =
        _G["SpellTimerSpellFrame" .. spell:GetName()] or
        CreateFrame("Frame", "SpellTimerSpellFrame" .. spell:GetName(), UIParent, "BFCDSpellFrameTemplate")
    t.ui = ui
    t:Bind(spell)
    return t
end
...
function CD:LoadConfig()
    BFCooldownDB = (BFCooldownDB and BFCooldownDB.isChangeFont and BFCooldownDB) or self:GetDefault()
    ignoredList = OO.SpellList:New(BFCooldownDB.ignored)
    coolingList = OO.SpellList:New(BFCooldownDB.cooling)
    succeedList = OO.SpellList:New(BFCooldownDB.succeed)
...
function CD:HookCooldown()
    ...
    if not ActionBarButtonEventsFrame then
        return
    end
    local active = {}
    ...
    local function cooldown_Update(self)
        ...
        if cooldown_ShouldUpdateTimer(self, start, duration) then
            CD:SetCooldown(self, start, duration)
        end
    end
    local abEventWatcher = CreateFrame("Frame")
    abEventWatcher:Hide()
    abEventWatcher:SetScript(
        "OnEvent",
        function(self, event)
    ...
    for _i = 1, succeedList:Count() do
        ...
        if spell then
            ...
            if spell:IsValid() then
                ...
                local ss = CD.Timer.ST:Create(spell);
                self.barFrame:Add(ss)
...
function CD:UNIT_SPELLCAST_SUCCEEDED(unitID, _, spellID)
    -- if unitID ~= "player" then
    --     return
    -- end
    print('spellsusss'..spellID)
    local newSpell = OO.Spell:New(spellID)
    newSpell:GetSpellInfo()
    -- if not newSpell.id then
    --     return
    -- end
    -- if ignoredList:HasSpell(newSpell) then
    --     return
    -- end
    ...
    view(succeedList.data[1])
end
function CD:PLAYER_ENTERING_WORLD()
    for _i = 1, coolingList:Count() do
        local _spell = coolingList:Get(_i)
        if _spell and not _spell:IsValid() or _spell:IsExpired() then
...
function CD:ToggleMiddle(tog)
    BFCooldownDB.center.config = tog
end
function CD:ToggleBar(tog)
    BFCooldownDB.bar = tog
end
function CD:SwitchActionStyle(tog)
    BFCooldownDB.ACTION.style = tog
end
function CD:ToggleSec(tog, type)
    BFCooldownDB[type].config = tog
end
function CD:ToggleSpecial(tog)
    BFCooldownDB.ACTION.special = tog
end
